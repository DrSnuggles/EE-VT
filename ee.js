...
window.ee = (function () {
  ...
  function vu(){
    ...
    */
    ee_vu0.style.height = 100 / 15 * ee.ym.player.voiceA.getvolume() +"vh";
    ee_vu1.style.height = 100 / 15 * ee.ym.player.voiceB.getvolume() +"vh";
    ee_vu2.style.height = 100 / 15 * ee.ym.player.voiceC.getvolume() +"vh";
...
/*
    ...
    ctx.beginPath();
    ctx.moveTo(0, height/2); // 0 level in mid of canvas
    ctx.lineTo(width, height/2);
    ctx.stroke();
*/
    var rescale = width / timeData.length;
    ctx.scale(scalingX, 1);	// fit all the data into the visible area

    ctx.lineWidth = 2;
    ctx.strokeStyle = 'rgb(96, 0, 0)';
    ctx.beginPath();
    ctx.moveTo(0, height/2); // 0 level in mid of canvas
    ctx.lineTo(timeData.length, height/2);
    ctx.stroke();

    ctx.lineWidth = 2;
    ctx.strokeStyle = 'rgb(0, 96, 0)';
    ctx.beginPath();
    ctx.moveTo(0, 0.5); // 0 level in mid of canvas
    for (var i = 1; i < timeData.length; i++){
      var mag = height - timeData[i] * scalingY; // Audacity also says that all data is above 0
      ctx.lineTo(i, mag);
    }
    ctx.stroke();
    ctx.restore(); // Juergen Wothke
    ...
    for (var x = risingEdge; x < timeData.length && x - risingEdge < width; x++)
      ctx.lineTo((x - risingEdge)*scalingX, height - timeData[x] * scalingY);
    */
